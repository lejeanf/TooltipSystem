...
namespace jeanf.tooltip
{
    ...
    public class InteractableToolTipController : ToolTip
    {
        ...
        // DisableTooltipVisibility() for code that needs it, and the ToolTipManager iPad-interruption flow
        ...
        [Tooltip("Seconds between placement re-evaluations. Higher = cheaper.")]
        [SerializeField] private float evaluationInterval = 0.2f;
        [Tooltip("Score bias kept for the current anchor so the tooltip doesn't flip-flop between near-equal positions.")]
        ...
        // bias which is why it can stay chosen over a marginally higher rival.
        ...
        {
            // No subscriber == no InteractableToolTipManager in the scene. That's a misconfiguration (every
        ...
        private void WarnNoPermissionManagerOnce()
        {
            ...
            Debug.LogWarning($"[InteractableToolTip '{name}'] No InteractableToolTipManager in the scene, so the " +
                             "gaze-permission gate denies every tooltip (none will maximize). Add an " +
                             "InteractableToolTipManager component to the scene.", this);
        }
        ...
        private bool IsWithinMinimizedRange()
        {
            ...
            if (_cameraTransform == null) return true;

            float sqr = (transform.position - _cameraTransform.position).sqrMagnitude;
